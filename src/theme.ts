...
// Copied verbatim from infinidraft's src/theme.ts - infinileague shares the
// same visual identity/design system as infinidraft (same account, same
// backend, same product family), so this file should stay in sync with
// infinidraft's rather than drift into its own palette. If infinidraft's
// theme changes, mirror the change here too.
...
export const theme = createTheme({
  ...
  components: {
    ...
    // this app's own fixed chrome in infinidraft (AppHeader, BottomNav,
    // etc.) - kept at the same value here for visual consistency even
    // though infinileague doesn't have that chrome yet.
    //
